...
export class Config {
  ...
  'server.port' = Number.parseInt(process.env.PORT);
  'jhipster.clientApp.name' = 'authentication-yooth-it';
  'jhipster.registry.password' = 'admin';
  'jhipster.security.authentication.jwt.base64-secret' = process.env.JWT_SECRET_KEY;
  ...
  'jhipster.mail.from' = 'yoothit@localhost';
  'jhipster.swagger.default-include-pattern' = '/api/.*';
  'jhipster.swagger.title' = 'yoothit API';
  'jhipster.swagger.description' = 'yoothit API documentation';
  ...
  'eureka.instance.appname' = 'yoothit';
  'eureka.instance.instanceId' = 'yoothit:${random.value}';
  ...
  'cloud.config.name' = 'yoothit';
  'cloud.config.profile' = 'prod';
  'loud.config.label' = 'master';

  'crypto.key-pwd' = process.env.PWD_CRYPTO_KEY;
  'crypto.iv-pwd' = process.env.PWD_CRYPTO_IV;

  'crypto.key-reset' = process.env.RESET_CRYPTO_KEY;
  'crypto.iv-reset' = process.env.RESET_CRYPTO_IV;

  'crypto.key-old-pwd' = process.env.OLD_PWD_CRYPTO_KEY;
  'crypto.iv-old-pwd' = process.env.OLD_PWD_CRYPTO_IV;

  'crypto.key-new-pwd' = process.env.NEW_PWD_CRYPTO_KEY;
  'crypto.iv-new-pwd' = process.env.NEW_PWD_CRYPTO_IV;

  'crypto.key-old-reset' = process.env.OLD_RESET_CRYPTO_KEY;
  'crypto.iv-old-reset' = process.env.OLD_RESET_CRYPTO_IV;

  'crypto.key-email' = process.env.EMAIL_CRYPTO_KEY;
  'crypto.iv-email' = process.env.EMAIL_CRYPTO_IV;
